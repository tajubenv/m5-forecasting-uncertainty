...
model_list <- list()
for (i in length(data_level_list)){
  
  model_list[[counter]] <- model_builder(data_level_list[[i]], calendar, ncol(L1), "validation")
}

output_results <- do.call(rbind, model_list)

output_results <- as.data.frame(output_results)

write.csv(output, "validation_submission.csv")
