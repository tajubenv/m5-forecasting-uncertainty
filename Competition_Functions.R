...
competition_run <- function(data){
  ...
  if(data == "evaluation"){
    sales_train <- read_csv(file.path(path, "sales_train_evaluation.csv"))
    outfile <- "evaluation_submission.csv"
  }
  else if(data == "validation"){
    sales_train <- read_csv(file.path(path, "sales_train_validation.csv"))
    outfile <- "validation_submission.csv"
  }
  sample_submission <- read_csv(file.path(path, "sample_submission.csv"))
  sell_prices <- read_csv(file.path(path, "sell_prices.csv"))
  
  col_tf <- colnames(sales_train) %>% stringr::str_detect("d_")
  vars <- colnames(sales_train)[col_tf]
  
  
  L1 <-  data_creator(sales_train)
  L2 <-  data_creator(sales_train, state_id)
  L3 <-  data_creator(sales_train, store_id)
  L4 <-  data_creator(sales_train, cat_id)
  L5 <-  data_creator(sales_train, dept_id)
  L6 <-  data_creator(sales_train, state_id, cat_id)
  L7 <-  data_creator(sales_train, state_id, dept_id)
  L8 <-  data_creator(sales_train, store_id, cat_id)
  L9 <-  data_creator(sales_train, store_id, dept_id)
  L10 <- data_creator(sales_train, item_id)
  L11 <- data_creator(sales_train, state_id, item_id)
  L12 <- data_creator(sales_train, item_id, store_id)
  ...
  calendar <- mutate(calendar, day = str_replace(d, "d_", ""),
    ...
    select(day, date, sporting_event, cultural_event, national_event, religious_event)
  ...
  colnames(final_result)[1] <- "id"
   
  write.csv(csv_results, outfile)
  return(TRUE)
}
